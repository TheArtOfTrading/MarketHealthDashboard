...
# TOTALRETURN = dividend-adjusted OHLC if you intentionally want adjusted returns.
...
class USSectorDashboardViewer:
    ...
    def draw_monthly_bar(self):
        ...
        self.ax.set_title(f"S&P 500 Sector ETF Monthly Open-to-Close Returns\n{month:%B %Y}")
    ...
    def draw_rolling_line(self):
        ...
        if self.period == "YTD":
            title = (
                f"S&P 500 Sector Rotation - YTD\n"
                f"January {end_month.year} open to {end_month:%b %Y} close"
            )
            y_label = "YTD return %"
        else:
            title = (
                f"S&P 500 Sector Rotation - {self.period} Month Rolling Open-to-Close Window\n"
...
SectorRotationLineViewer = USSectorDashboardViewer


class MonthlySectorBarViewer(USSectorDashboardViewer):
...
def plot_us_sector_dashboard(
    monthly_opens: pd.DataFrame,
    monthly_closes: pd.DataFrame,
    monthly_returns: pd.DataFrame,
    window_months: int = 12
):
    viewer = USSectorDashboardViewer(
        monthly_opens,
        monthly_closes,
        monthly_returns,
        window_months=window_months
...
def plot_latest_month(returns: pd.DataFrame):
    viewer = MonthlySectorBarViewer(returns)
    plt.show()
    return viewer
...
def plot_monthly_heatmap(returns: pd.DataFrame, months: int = 12):
    ...
    ax.set_title(f"S&P 500 Sector ETF Monthly Open-to-Close Returns - Last {months} Completed Months")
    ax.set_yticks(range(len(recent.index)))
    ax.set_yticklabels(recent.index)
...
if __name__ == "__main__":
    ...
    viewer = plot_us_sector_dashboard(
        monthly_opens,
        monthly_closes,
        monthly_returns,
        window_months=WINDOW_MONTHS
